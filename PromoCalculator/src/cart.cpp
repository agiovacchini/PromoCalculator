...
Cart::Cart( string p_base_path, uint32_t p_number, unsigned int p_action, bool p_dummy_rcs )
{
    ...
    tmpTransactionFile.open( tmp_transaction_file_name, fstream::app );
...
void Cart::writeTransactionRow(const string &row ) const {
    std::ofstream tmpTransactionFile;
    tmpTransactionFile.open( tmp_transaction_file_name, fstream::app );
    tmpTransactionFile << to_iso_string(boost::posix_time::microsec_clock::universal_time()) << "," << row << "\n";
    tmpTransactionFile.close() ;
}
...
bool Cart::updateLocalItemMap(Item p_item, Department p_dept)
{
    bool result;
...
long Cart::removeItemByBarcode( const Item& p_item, uint64_t p_barcode, int64_t p_price, int64_t p_discount )
{
    long rc;
    if ( this->getState() == CART_STATE_TMPFILE_LOADING || this->getState() == CART_STATE_READY_FOR_ITEM )
    {
...
long Cart::persist( )
{
    ...
    for(barcodesRows iterator = barcodes_map.begin(); iterator != barcodes_map.end(); iterator++)
    {
...
long Cart::sendToPos( uint32_t p_pos_number, string p_scan_in_path, string p_store_id )
{
    if (this->getState() == CART_STATE_READY_FOR_ITEM)
    {
        ...
        uint64_t rowBarcode ;
        long qty = 0 ;
        string scanInTmpFileName = (boost::format("%s/POS%03lu.TMP") % p_scan_in_path % p_pos_number).str();
        string scanInFileName = (boost::format("%s/POS%03lu.IN") % p_scan_in_path % p_pos_number).str();
        ...
        tempStringStream << loy_cards_map.begin()->second ;
    ...
    if (p_with_barcodes)
    {
        ...
        for(itemRows iterator = cart_items_map.begin(); iterator != cart_items_map.end(); iterator++)
        {
            ...
            if (qty > 0)
            {
                ...
                tempStringStream << "\"item\":{\"intCode\":" << rowIntCode << ",\"description\":\"" << p_all_items_map[rowIntCode].getDescription() << "\",\"qty\":" << qty << "}" << endl;
                firstRow = false ;
            }
        }
    }
...
void Cart::setRescan( bool p_rescan_required) {
